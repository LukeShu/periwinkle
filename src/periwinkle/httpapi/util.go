...
func safeDecodeJSON(in interface{}, out interface{}) *he.Response {
	...
	if !ok {
		ret := rfc7231.StatusUnsupportedMediaType(heutil.NetString("PUT and POST requests must have a document media type"))
		return &ret
	}
	var tmp interface{}
	err := decoder.Decode(&tmp)
	if err != nil {
		ret := rfc7231.StatusUnsupportedMediaType(heutil.NetPrintf("Couldn't parse: %v", err))
		return &ret
	}
	...
	if err != nil {
		ret := rfc7231.StatusUnsupportedMediaType(heutil.NetPrintf("Request body didn't have expected structure (field had wrong data type): %v", err))
		return &ret
	}
