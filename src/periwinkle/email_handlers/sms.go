...
func HandleSMS(r io.Reader, name string, db *gorm.DB , config cfg.Cfg) uint8 {
	message, err := mail.ReadMessage(r)
	if err != nil {
		return postfixpipe.EX_NOINPUT
	}
	status, err := sender(*message, name, db, config)
	if err != nil {
		log.Println(err)
		return postfixpipe.EX_NOINPUT
	}
...
// Error.
func sender(message mail.Message, to string, db *gorm.DB, config cfg.Cfg) (status string, err error) {
	...
	v.Set("To", sms_to)
	v.Set("Body", sms_body)
	v.Set("StatusCallback", "http://"+config.WebRoot+"/callbacks/twilio-sms")
